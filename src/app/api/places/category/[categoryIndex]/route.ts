...
export const fetchFilteredCategoryData = async (
  ...
  filters: { filter1?: boolean; filter2?: boolean; filter3?: boolean; filter4?: boolean }
): Promise<any[]> => {
  // 환경 변수 유효성 검사
  if (!API_BASE_URL) {
  ...
  try {
    ...
    // filters 처리
    if (filters.filter1 !== undefined) queryParams.append('filter1', String(filters.filter1));
    if (filters.filter2 !== undefined) queryParams.append('filter2', String(filters.filter2));
    if (filters.filter3 !== undefined) queryParams.append('filter3', String(filters.filter3));
    if (filters.filter4 !== undefined) queryParams.append('filter4', String(filters.filter4));
